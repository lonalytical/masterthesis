...
$W_j$ is correlated with $\bar{X}_{.j}$ but not with $X_{ij}$ (see Equation @eq-W), and $Var(W_j)$ is set to 1, which yields:
$$
Var(\text{fixed})
...
Using the decomposition of $X_{ij}$ introduced in Section @sec-dgm, $X_{ij} = u_{Xj} + e_{Xij}$, the covariance term can be rewritten and expanded as
