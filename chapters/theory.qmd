...
This and the models explained below are illustrated in Figure \ref{fig:mlmplots}.

```{=latex}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/plotmlm.png}
  \caption{Illustration of common multilevel model specifications.}
  \label{fig:mlmplots}
  \caption*{\textit{Note.} Top row: empty model, random intercept model, and random slope model. Bottom row: group-level relationships between group means of the predictor $\bar{X}_{.j}$ and the group-specific intercepts $\beta_{0j}$.}
\end{figure}
```
