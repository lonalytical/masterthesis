...
In the preceding chapter, the theoretical foundation and parameters of the multilevel model were introduced. In practice, however, these parameters are unknown and only exist in the theoretical realm as ‘true’ values. The fixed effects $\gamma$ and variance components $\sigma^2$ and $\tau^2$ must therefore be estimated from observed data. Based on these estimates, inferences about the underlying population parameters are made. In frequentist approaches, estimation procedures of multilevel models are based on 'asymptotic theory' and assume a sufficiently large sample size. In practice, this assumption is often difficult to meet, especially because estimation depends not only on the number of individual observations (Level 1), but also on the number of clusters (Level 2), which are often difficult or costly to obtain [@mcneishcoll]. Fortunately, statistics has solutions at hand for those cases. The main challenges of small-sample multilevel models and possible solutions are presented in the following section.

### Main problems
In multilevel modeling, statistical inference is primarily determined by three key components. If these are estimated accurately and appropriately specified, the inferential machinery of the model generally performs reliably [@elff]:
